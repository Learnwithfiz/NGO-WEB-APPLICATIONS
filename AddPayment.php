...
<body>
   ...
   <button class="btn btn-primary p-3 m-5 "><a style="border:none;color:wheat !important" href="ModerateDashboard.php">Home</a></button>
                ...
                <div class="form-div" style="padding: 20px;">
                    ...
                    <div class="row">
                      <div class="col-lg-4">
                        <span class="span"> Name of Client  ..</span>
                        <div>
                           <input type="text" id="name" value="<?php echo $name;   ?>"  name="Name" placeholder="Your name...." required>
                        </div>
                      </div>
                      ...
                      <div class="col-lg-4">
                        ...
                        <div>
                           <input type="text" id="phone"  value="<?php echo $email;   ?>"  name="phone" required placeholder="Your phone number...">
                        </div>
                      </div>
                      <div class="col-lg-4">
                        <span class="span">Account Number</span>
                        <div>
                        <input type="text"  id="acc_num"   value="<?php echo $phone;   ?>" maxlength="11"   name="acc_num" placeholder="acc_num" required >
                        </div>
                    ...
                    <div class="row">
                       ...
                       <div class="col-lg-4">
                        <span class="span">Payment Date </span>
                        <div>
                        <input type="date"  id="pay_date"  name="date"  required >
                        </div>

                        
                       </div>
                       <div class="col-lg-4">
                       <span class="span"> Payment  (Taka) / Money</span>
                       <div>
                       <input id="payment_poriman" type="text"value="<?php echo $work;   ?>"    name="payment_poriman" placeholder="describe work" required >
                        <input type="hidden" id="hidden_user_id">
                       <button id="btn" onclick="SetData()" name="submit"> Submit <i class="fas fa-long-arrow-alt-right"></i></button>
                        </div>
                       </div>
                        ...
                        <?php
                          ...
                          //  $d = ;
                            ...
                            while ($row = mysqli_fetch_assoc($e)) {
                                $user_id = $row['user_id'];
                                $payment_poriman = $row['payment_poriman'];
                                if (!isset($totalQtyArr[$user_id])) {
                                    $totalQtyArr[$user_id] = 0;
                                }
                                $totalQtyArr[$user_id] += $payment_poriman;
                            }
                            // echo "<pre>";
                            // print_r($totalQtyArr);
                           ...
                           $totalQty =     $row['total_paymnet'] - $totalQtyArr[$row['id']];
                         
                            ?>
                            

                            <tr onclick="setFormData(<?php echo $row['id'] ?>)">
                                <td><?php echo $i ?></td>
                                ...
                                <td><?php  echo $totalQty; // $row['total_paymnet']; // $row['total_paymnet']  ?></td>
...
<!--javascript ends  -->
   ...
   <script>
      ...
      $.ajax({
        ...
        success:function(res)
        {
          var user = JSON.parse(res);
           $("#name").val(user.Name);
           $("#acc_num").val(user.acc_num);
           $("#pay_date").val(user.date);
           $("#payment_poriman").val(user.total_paymnet);
    ...
    function SetData()
    {
       ...
       var payment_poriman = $("#payment_poriman").val();
    //    if(name==''){alert('name is required') } else { return ; }
    //    if(acc_num==''){alert('acc_num is required') } else { return ; }
    //    if(payment_poriman==''){alert('payment_poriman is required') } else { return ; }
       ...
       $.ajax({
        method:"POST",
        url:"sendPay.php",
        data:{user_id:user_id,payment_poriman:payment_poriman},
        success:function(res){
            alert(res)
        }
       })
