...
if($date)
{
    $dateCond="AND a.curren_date BETWEEN '$date' " ; 
}else{
    $dateCond="";
}
$todateCond = "";
if($todate)
{
    $todateCond="AND   '$todate' " ; 
}else{
    $todateCond="";
}
 $sql = "SELECT * FROM add_payment a , client b WHERE a.user_id = b.id   $acc_cond $emailcond $phoneCond $dateCond $todateCond";
